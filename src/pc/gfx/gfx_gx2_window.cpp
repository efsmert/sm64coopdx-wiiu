...
static bool sWiiUDebugClearLogged = false;
static bool sWiiUDebugForegroundLogged = false;
...
static void gfx_gx2_window_set_keyboard_callbacks(UNUSED kb_callback_t on_key_down, UNUSED kb_callback_t on_key_up,
    UNUSED void (*on_all_keys_up)(void), UNUSED void (*on_text_input)(char*), UNUSED void (*on_text_editing)(char*, int))
{
}
...
static bool gfx_gx2_window_start_frame(void)
{
    ...
    // Keep a visible clear active during Wii U black-screen triage so we can
    // distinguish "scene not drawing" from "frame not presenting".
    ...
    GX2ClearColor(&g_color_buffer, 0.07f, 0.18f, 0.32f, 1.0f);
    if (!sWiiUDebugClearLogged) {
        sWiiUDebugClearLogged = true;
        WHBLogPrint("gfx: wiiu debug color clear active");
    }
...
{
    static bool sLoggedSwapBegin = false;
    if (!sLoggedSwapBegin) {
        sLoggedSwapBegin = true;
        WHBLogPrint("gfx: wiiu swap_buffers_begin");
    }
...
{
    static bool sLoggedSwapEnd = false;
    if (!sLoggedSwapEnd) {
        sLoggedSwapEnd = true;
        WHBLogPrint("gfx: wiiu swap_buffers_end enter");
    }
    uint32_t swap_count, flip_count;
    int64_t prev_flip, prev_vsync;
    ...
    if (sLoggedSwapEnd) {
        static bool sLoggedSwapEndDone = false;
        if (!sLoggedSwapEndDone) {
            sLoggedSwapEndDone = true;
            WHBLogPrint("gfx: wiiu swap_buffers_end done");
        }
    }
}
...
static void gfx_gx2_window_set_clipboard_text(UNUSED const char* text)
{
}

static void gfx_gx2_window_set_cursor_visible(UNUSED bool visible)
{
}
...
static void gfx_gx2_window_set_window_title(UNUSED const char* title)
{
}
